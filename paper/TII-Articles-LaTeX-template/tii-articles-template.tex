...
如 Fig.~\ref{fig:methodology-framework} 所示，本文介绍一种基于知识图谱增强的数控编程方案。整体流程由四个相互衔接的阶段组成：1) 工作流组织与任务分解；2) Offline Template Construction；3) LLM Agent \& Reasoning；4) Synthesis \& Verification。这套方案结合多阶段协同推理与迭代校验逻辑，能够将自然语言形式的加工需求转化为安全可靠的可执行程序。本节将根据上述阶段依次介绍核心方法及实现细节。



\begin{figure}[!t]
\centering
\includegraphics[width=0.98\columnwidth]{Overall workflow of the LLM-KG CNC programming system.png}
\caption{Overall workflow of the LLM-KG CNC programming system.}
\label{fig:methodology-framework}
\end{figure}

\begin{algorithm}[!t]
\caption{Closed-loop knowledge-graph-enhanced G-code generation (P1--P3)}
\label{alg:closed-loop-gcode}
\begin{algorithmic}[1]
\REQUIRE User instruction $u$, context $c$, knowledge graph $\mathcal{G}$ (Neo4j), threshold $\tau$
\ENSURE Final G-code $y$ and validation report $R$
\STATE \textbf{Offline:} curate templates and update Neo4j (\textit{SubProcess.code})
\STATE Normalize $(u', c') \leftarrow \text{Normalize}(u,c)$
\STATE \textbf{P3:} $(p,f,conf) \leftarrow \text{IntentParse}(u', c')$
\IF{$conf < \tau$}
  \STATE $(p,f) \leftarrow \text{SelfReflect}(u', c', p,f)$
\ENDIF
\STATE \textbf{P1:} $C \leftarrow \text{KGQuery}(p,f,c';\mathcal{G})$ \hfill (Cypher candidates)
\STATE \textbf{P2:} $prompt \leftarrow \text{AssemblePrompt}(u',c',C,\text{session})$
\STATE $m^{\ast} \leftarrow \text{SelectTemplate}(prompt, C)$
\STATE $y \leftarrow \text{Synthesize}(m^{\ast}, c')$ \hfill (execute \textit{SubProcess.code})
\STATE $R \leftarrow \text{Verify}(y)$ \hfill (syntax/range/LLM)
\WHILE{$R.\text{pass} = \text{False}$}
  \STATE $(u',c') \leftarrow \text{Refine}(u',c',R)$
  \STATE \textbf{P1--P3} repeat to update $m^{\ast}$ and $y$
  \STATE $R \leftarrow \text{Verify}(y)$
\ENDWHILE
\RETURN $(y, R)$
\end{algorithmic}
\end{algorithm}

\subsection{Offline Template Construction}

The offline stage aims to solidify domain expertise into reusable and auditable artifacts. Specifically, the process taxonomy (six major process categories and their sub-processes) is represented as \textit{Process/SubProcess} nodes with \textit{INCLUDES} relations, while executable templates are stored in Neo4j as code strings in \textit{SubProcess.code}. Template authoring is human-in-the-loop: experts review machining logic and controller constraints, optionally grounding decisions with documentation retrieval (e.g., GraphRAG references). The final templates are committed into the graph database via Cypher updates, providing stable candidates for online retrieval.

\subsection{LLM Agent \& Reasoning (P1--P3)}

Given a user instruction $u$ and runtime context $c$ (machine/controller settings and session history), the agent first performs intent routing to distinguish machining tasks from knowledge queries. For machining tasks, it predicts the process type $p$ and sub-process $f$ with a confidence score $conf$. A self-reflection mechanism is triggered when $conf<\tau$ (with $\tau=0.7$ in our implementation), prompting the model to re-examine potential keyword omissions and ambiguities.

The agent maintains a session memory that stores collected parameters and dialog history. The task planner decomposes the query and orchestrates tool calls. We use three staged plans to keep the pipeline controllable: P1 invokes tools and performs Cypher-based KG retrieval; P2 assembles a complete prompt by combining KG results, session parameters, and controller rules; and P3 drives the final reasoning for selection and output.

\subsection{KG Retrieval \& G-code Synthesis}

In P1, the KG retriever queries Neo4j and retrieves candidate templates (i.e., \textit{SubProcess.code}) associated with the identified process. The template selector ranks candidates using a multi-criteria strategy (LLM compatibility, parameter coverage, semantic consistency, and usage prior). After selecting $m^{\ast}$, the synthesis module executes the code stored in \textit{SubProcess.code} to instantiate the corresponding process class and generate the G-code program $y$. Optional post-processing can be applied to ensure controller-specific formatting.

\subsection{Synthesis \& Three-level Verification with Visual Feedback}

To ensure safety and executability, we adopt a three-level verification: (i) \textit{syntax rules} to catch malformed lines and illegal command patterns, (ii) \textit{parameter range checks} for critical fields (e.g., feed rate and spindle speed), and (iii) \textit{LLM deep verification} for semantic and process-level issues, producing structured issues and improvement suggestions. The final verification score aggregates the three dimensions with safety prioritized:
\begin{equation}
    s_{\text{overall}} = 0.5\,s_{\text{safety}} + 0.25\,s_{\text{eff}} + 0.25\,s_{\text{std}}.
\end{equation}
If verification fails, the report $R$ provides evidence for refinement and triggers another iteration of P1--P3. In addition, an external Unity-based Sim2Real UI offers lightweight visual inspection; its visual feedback can assist the agent in iterative refinement when necessary.
